...
use crate::object_store::{Directory, HandleOptions, ObjectDescriptor, ObjectStore, StoreObjectHandle};
...
impl FuseFs {
    ...
    pub async fn default_store(&self) -> Arc<ObjectStore> {
        let root_volume = root_volume(self.fs.clone()).await.expect("root_volume failed");
        let store =
            root_volume.volume(DEFAULT_VOLUME_NAME, self.crypt.clone()).await.expect("FUSE volume failed");
        store
    }
    ...
    pub async fn get_object_handle(&self, object_id: u64) -> Result<StoreObjectHandle<ObjectStore>> {
        let store = self.default_store().await;
        let handle_result = ObjectStore::open_object(
            &store,
            object_id,
    ...
    pub async fn create_object_attr(&self, object_id: u64, object_type: ObjectDescriptor) -> Result<FileAttr> {
        let properties = self.get_object_properties(object_id).await?;
        match object_type {
            ObjectDescriptor::Directory => Ok(create_dir_attr(object_id, properties.allocated_size, properties.creation_time, properties.modification_time, DEFAULT_FILE_MODE)),
            ObjectDescriptor::File => Ok(create_file_attr(object_id, properties.allocated_size, properties.creation_time, properties.modification_time, DEFAULT_FILE_MODE)),
